...
class Delete(Verb):
    ...
    def action(self, allow_external_connections=True):
        ...
        if isinstance(self.object, str):
            ...
            model = apps.get_model("reeltalk.User")
...
class Undo(Verb):
    ...
    def action(self, allow_external_connections=True):
        ...
        if self.object.type == "Follow":
            model = apps.get_model("reeltalk.UserFollows")
            obj = self.object.to_model(
                model=model,
                save=False,
                allow_create=False,
            ...
            if not obj:
                ...
                model = apps.get_model("reeltalk.UserFollowRequest")
                obj = self.object.to_model(
                    model=model,
                    save=False,
                    allow_create=False,
...
class Reject(Verb):
    ...
    def action(self, allow_external_connections=True):
        ...
        for model_name in ["UserFollowRequest", "UserFollows", None]:
            model = apps.get_model(f"reeltalk.{model_name}") if model_name else None
...
class Move(Verb):
    ...
    def action(self, allow_external_connections=True):
        ...
        if object_is_user:
            model = apps.get_model("reeltalk.MoveUser")
