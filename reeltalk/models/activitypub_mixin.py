...
from reeltalk.settings import USER_AGENT, PAGE_LENGTH
from reeltalk.signatures import make_signature, make_digest
from reeltalk.tasks import app, BROADCAST
from reeltalk.models.fields import ImageField, ManyToManyField
...
class ActivitypubMixin:
    ...
    def broadcast(self, activity, sender, software=None, queue=BROADCAST):
        ...
        site_model = apps.get_model("reeltalk.SiteSettings", require_ready=True)
        try:
    ...
    def get_recipients(self, software=None) -> list[str]:
        ...
        if privacy != "direct":
            ...
            if software:
                queryset = queryset.filter(reeltalk_user=software == "reeltalk")
...
class CollectionItemMixin(ActivitypubMixin):
    ...
    def broadcast(self, activity, sender, software="reeltalk", queue=BROADCAST):
        """only send book collection updates to other reeltalk instances"""
