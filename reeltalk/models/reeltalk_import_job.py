"""Import a user from another Reeltalk instance"""
...
from reeltalk.models.job import Job, ParentJob, ChildJob, ParentTask, SubTask
...
class ReeltalkImportJob(ParentJob):
    """entry for a specific request for importing a reeltalk user backup"""
    ...
    def pending_item_count(self):
        ...
        status = ReeltalkImportJob.Status
...
class UserImportBook(ChildJob):
    ...
    def complete_job(self):
        """Report to ReeltalkImportJob that the job has completed.
        ...
        parent = ReeltalkImportJob.objects.get(id=self.parent_job.id)
...
class ImportUserTask(ParentTask):
    ...
    def before_start(self, task_id, args, kwargs):
        ...
        job = ReeltalkImportJob.objects.get(id=kwargs["job_id"])
        job.task_id = task_id
...
class UserImportSubTask(SubTask):
    ...
    def before_start(self, task_id, args, kwargs):
        ...
        model = apps.get_model(f"reeltalk.{kwargs['job_type']}", require_ready=True)
...
def start_import_task(**kwargs):
    ...
    job = ReeltalkImportJob.objects.get(id=kwargs["job_id"])
    archive_file = job.reeltalkimportjob.archive_file
...
def import_book_task(**kwargs):
    ...
    try:
        ...
        required = task.parent_job.reeltalkimportjob.required
    ...
    if "include_comments" in job.reeltalkimportjob.required:
    ...
    if "include_quotations" in job.reeltalkimportjob.required:
    ...
    if "include_reviews" in job.reeltalkimportjob.required:
