"""the particulars for this instance of ReelTalk"""
...
from .base_model import ReelTalkModel, new_access_code
...
class SiteModel(models.Model):
    ...
    def raise_not_editable(self, viewer: User) -> None:
        ...
        if viewer.has_perm("reeltalk.edit_instance_settings"):
            return
...
class SiteInvite(models.Model):
    ...
    def raise_not_editable(self, viewer: User) -> None:
        """Admins only"""
        if viewer.has_perm("reeltalk.create_invites"):
            return
        raise PermissionDenied()
...
class InviteRequest(ReelTalkModel):
    ...
    def raise_not_editable(self, viewer: User) -> None:
        ...
        if not self.id or viewer.has_perm("reeltalk.create_invites"):
            return
        raise PermissionDenied()
