...
from reeltalk.connectors.connector_manager import search
...
class CleanUpUserExportFilesJob(ParentJob):
    ...
    def start_job(self):
        ...
        import_jobs = models.ReeltalkImportJob.objects.filter(
            complete=True, updated_date__lt=self.expiry_date
        )
...
def delete_user_export_file_task(**kwargs):
    ...
    if kwargs.get("import_id"):
        file = models.ReeltalkImportJob.objects.get(id=kwargs["import_id"])
