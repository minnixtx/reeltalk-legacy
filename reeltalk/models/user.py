...
from .base_model import ReelTalkModel, DeactivationReason, new_access_code
...
class User(OrderedCollectionPageMixin, AbstractUser):
    ...
    reeltalk_user = fields.BooleanField(default=True)
    ...
    def to_outbox(self, filter_type=None, **kwargs):
        ...
        if filter_type:
            filter_class = apps.get_model(f"reeltalk.{filter_type}", require_ready=True)
    ...
    def save(self, *args, update_fields: Optional[Iterable[str]] = None, **kwargs):
        ...
        with transaction.atomic():
            ...
            try:
                group = (
                    apps.get_model("reeltalk.SiteSettings")
                    .objects.get()
    ...
    @property
    def local_path(self):
        """this model doesn't inherit reeltalk model, so here we are"""
    ...
    def raise_not_editable(self, viewer):
        ...
        if self == viewer or viewer.has_perm("reeltalk.moderate_user"):
            return
...
class KeyPair(ActivitypubMixin, ReelTalkModel):
...
def set_remote_server(user_id, allow_external_connections=False):
    ...
    if user.reeltalk_user and user.outbox:
...
def get_remote_reviews(outbox):
    """ingest reviews by a new remote reeltalk user"""
