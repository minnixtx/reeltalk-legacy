...
from reeltalk.isbn.isbn import hyphenator_singleton as hyphenator
...
from reeltalk.utils.db import format_trigger, add_update_fields
...
class BookDataModel(ObjectMixin, ReelTalkModel):
    ...
    def broadcast(self, activity, sender, software="reeltalk", **kwargs):
        """only send book data updates to other reeltalk instances"""
            ...
            pgtrigger.Trigger(
                ...
                func=format_trigger(
                    ...
                    WITH author_names AS (
                        SELECT array_to_string(reeltalk_author.name || reeltalk_author.aliases, ' ') AS name_and_aliases
                            FROM reeltalk_author
                        LEFT JOIN reeltalk_book_authors
                            ON reeltalk_author.id = reeltalk_book_authors.author_id
                        WHERE reeltalk_book_authors.book_id = new.id
                    )
                    SELECT
...
class Series(OrderedCollectionMixin, BookDataModel):
    ...
    def raise_not_editable(self, viewer):
        if not viewer.has_perm("reeltalk.edit_book"):
...
class SeriesBook(CollectionItemMixin, ReelTalkModel):
