...
from reeltalk.importers import GoodreadsImporter
from reeltalk.models.import_job import import_item_task
...
class UserImportStatus(View):
    ...
    def get(self, request, job_id):
        """status of an import job"""
        job = get_object_or_404(models.ReeltalkImportJob, id=job_id)
        if job.user != request.user:
            raise PermissionDenied()
...
def stop_user_import(request, job_id):
    """scrap that"""
    job = get_object_or_404(models.ReeltalkImportJob, id=job_id, user=request.user)
    job.stop_job()
