...
from reeltalk.importers import (
    ReeltalkImporter,
    ReeltalkBooksImporter,
    CalibreImporter,
...
class Import(View):
    ...
    def post(self, request):
        ...
        if source == "ReelTalk":
            importer = ReeltalkBooksImporter()
...
class UserImport(View):
    ...
    def get(self, request, invalid=False):
        ...
        jobs = ReeltalkImportJob.objects.filter(user=request.user).order_by(
            "-created_date"
        )
        paginated = Paginator(jobs, PAGE_LENGTH)
        page = paginated.get_page(request.GET.get("page"))
...
def user_import_available(user: models.User) -> Optional[tuple[datetime, int]]:
    ...
    jobs = ReeltalkImportJob.objects.filter(user=user).order_by("-created_date")
    site = models.SiteSettings.objects.get()
...
def get_average_user_import_time() -> float:
    ...
    last_week = timezone.now() - datetime.timedelta(days=7)
    recent_avg = (
        models.ReeltalkImportJob.objects.filter(
