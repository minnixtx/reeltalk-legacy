...
from reeltalk.settings import PAGE_LENGTH, USE_AZURE
...
class ImportList(View):
    ...
    def get(self, request, status="active"):
        ...
        user_imports = models.ReeltalkImportJob.objects.filter(
            complete=complete
...
def set_user_import_completed(request, import_id):
    ...
    import_job = get_object_or_404(models.ReeltalkImportJob, id=import_id)
    import_job.stop_job()
    return redirect("settings-imports")
