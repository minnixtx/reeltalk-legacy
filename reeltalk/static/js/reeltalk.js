...
let ReelTalk = new (class {
    ...
    initOnDOMLoaded() {
        ...
        window.addEventListener("DOMContentLoaded", function () {
            ...
            document
                .querySelectorAll('input[type="file"]')
                .forEach(reeltalk.disableIfTooLarge.bind(reeltalk));
            document.querySelectorAll("[data-copytext]").forEach(reeltalk.copyText.bind(reeltalk));
            document
                .querySelectorAll("[data-copywithtooltip]")
                .forEach(reeltalk.copyWithTooltip.bind(reeltalk));
            document
                .querySelectorAll(".modal.is-active")
                .forEach(reeltalk.handleActiveModal.bind(reeltalk));
        });
    }
        ...
        setTimeout(
            function () {
                fetch("/api/updates/" + counter.dataset.poll)
                    ...
                    .then((data) => {
                        reeltalk.updateCountElement(counter, data);
                        reeltalk.polling(counter);
                    })
                    .catch(() => {
                        reeltalk.polling(counter, delay * 1.1);
                    });
            },
            delay,
            counter
    ...
    interact(event) {
        ...
        relatedforms.forEach((relatedForm) => {
            ...
            if (isHidden) {
                reeltalk.classShow(relatedForm);
            } else {
                reeltalk.classHide(relatedForm);
            }
        });
