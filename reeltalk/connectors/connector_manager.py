...
from reeltalk.connectors import abstract_connector
from reeltalk.settings import SEARCH_TIMEOUT
from reeltalk.tasks import app, CONNECTORS
...
def get_connectors() -> Iterator[abstract_connector.AbstractConnector]:
    ...
    if models.SiteSettings.get().disable_federation:
        queryset = queryset.exclude(connector_file="reeltalk_connector")
...
def load_connector(
    ...
    connector = importlib.import_module(
        f"reeltalk.connectors.{connector_info.connector_file}"
    )
    return connector.Connector(connector_info.identifier)  # type: ignore[no-any-return]


@receiver(signals.post_save, sender="reeltalk.FederatedServer")
def create_connector(
    ...
    **kwargs: Any,
    ...
    """create a connector to an external reeltalk server"""
    if instance.application_type == "reeltalk":
