...
from reeltalk.connectors.abstract_connector import AbstractMinimalConnector
...
class SearchVectorUpdates(TestCase):
    ...
    def setUpTestData(cls):
        ...
        with connection.cursor() as cursor:
            cursor.execute(
                """
                ALTER SEQUENCE reeltalk_author_id_seq       RESTART WITH 20;
                ALTER SEQUENCE reeltalk_book_authors_id_seq RESTART WITH 300;
                """
            )
    ...
    def test_search_after_author_add_remove_sql(self):
        ...
        # reeltalk_book_authors is removed (probably because they trigger the one
        # in reeltalk_book directly). Here we make sure to exercise the former.
        new_author = models.Author.objects.create(name="Mozilla")

        with connection.cursor() as cursor:
            cursor.execute(
                "DELETE FROM reeltalk_book_authors WHERE book_id = %s",
                [self.edition.id],
            )
        self.assertFalse(self._search("Name"))
        self.assertFalse(self._search("Mozilla"))
        ...
        with connection.cursor() as cursor:
            cursor.execute(
                "INSERT INTO reeltalk_book_authors (book_id,author_id) VALUES (%s,%s)",
