...
from reeltalk.activitypub.base_activity import (
    ActivityObject,
    resolve_remote_id,
    set_related_field,
    get_representative,
...
class BaseActivity(TestCase):
    ...
    def test_to_model_one_to_many(self, *_):
        ...
        with (
            patch("reeltalk.activitypub.base_activity.set_related_field.delay"),
            patch("reeltalk.models.status.Status.ignore_activity") as discarder,
        ):
    ...
    def test_do_not_raise_error_on_410(self, *_):
        ...
        with self.assertLogs(level="DEBUG") as logger:
            ...
            self.assertEqual(
                logger.output,
                [
                    "WARNING:reeltalk.activitypub.base_activity:request for object dropped because it is gone (410) - remote_id: https://example.com/user/mouse"
                ],
            )
    ...
    @patch("reeltalk.activitypub.base_activity.r.set")
    @patch("reeltalk.activitypub.base_activity.time.sleep")
    ...
    @patch("reeltalk.activitypub.base_activity.r.set")
    @patch("reeltalk.activitypub.base_activity.r.delete")
    @patch("reeltalk.activitypub.base_activity.time.sleep")
    ...
    @patch("reeltalk.activitypub.base_activity.r.delete")
    @patch("reeltalk.activitypub.base_activity.time.sleep")
    @patch("reeltalk.activitypub.base_activity.time.monotonic")
