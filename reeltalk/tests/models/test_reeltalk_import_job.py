...
from reeltalk.models import reeltalk_import_job


class ReeltalkImport(TestCase):
    ...
    def setUpTestData(self):
        ...
        self.job = reeltalk_import_job.ReeltalkImportJob.objects.create(
    ...
    def test_update_user_profile(self):
        ...
        with (
            ...
            with (
                ...
                ReeltalkTarFile.open(mode="r:gz", fileobj=fileobj) as tarfile,
            ):
                models.reeltalk_import_job.update_user_profile(
    ...
    def test_update_user_settings(self):
        ...
        with (
            ...
            models.reeltalk_import_job.update_user_settings(
    ...
    def test_upsert_saved_lists_existing(self):
        ...
        with patch("reeltalk.activitypub.base_activity.resolve_remote_id"):
            models.reeltalk_import_job.upsert_saved_lists(
    ...
    def test_follow_relationship(self):
        ...
        task = reeltalk_import_job.UserImportRelationship.objects.create(
            parent_job=self.job,
            relationship="follow",
        ...
        with (
            ...
            patch("reeltalk.models.activitypub_mixin.broadcast_task.apply_async"),
            patch("reeltalk.activitypub.resolve_remote_id", return_value=self.rat_user),
        ):
            reeltalk_import_job.import_user_relationship_task(child_id=task.id)
    ...
    def test_import_book_task_existing_author(self):
        ...
        with open(self.archive_file_path, "rb") as fileobj:
            ...
            self.job.save()
        task = reeltalk_import_job.UserImportBook.objects.create(
        ...
        # run the task
        reeltalk_import_job.import_book_task(
    ...
    def test_block_relationship(self):
        ...
        with (
            patch("reeltalk.suggested_users.remove_suggestion_task.delay"),
            patch("reeltalk.activitystreams.remove_user_statuses_task.delay"),
            patch("reeltalk.lists_stream.remove_user_lists_task.delay"),
            patch("reeltalk.models.activitypub_mixin.broadcast_task.apply_async"),
            patch(
                "reeltalk.activitypub.resolve_remote_id", return_value=self.badger_user
            ),
        ):
            reeltalk_import_job.import_user_relationship_task(child_id=task.id)
    ...
    def test_upsert_readthroughs(self):
        ...
        reeltalk_import_job.upsert_readthroughs(
    ...
    def test_get_or_create_review(self):
        ...
        task = reeltalk_import_job.UserImportPost.objects.create(
            parent_job=self.job,
            book=self.book,
        ...
        with patch("reeltalk.models.reeltalk_import_job.is_alias", return_value=True):
            reeltalk_import_job.upsert_status_task(child_id=task.id)
    ...
    def test_get_or_create_quote_unauthorized(self):
        ...
        with patch("reeltalk.models.reeltalk_import_job.is_alias", return_value=False):
            reeltalk_import_job.upsert_status_task(child_id=task.id)
        self.assertEqual(
            models.Quotation.objects.filter(user=self.local_user).count(), 0
        )
    ...
    def test_upsert_list_existing(self):
        ...
        with (
            ...
            reeltalk_import_job.upsert_lists(
                self.local_user,
                other_book.id,
    ...
    def test_upsert_shelves_existing(self):
        ...
        with (
            ...
            reeltalk_import_job.upsert_shelves(
    ...
    def test_update_followers_address(self):
        ...
        new_followers = reeltalk_import_job.update_followers_address(user, followers)
    ...
    def test_is_alias(self):
        ...
        with patch(
            "reeltalk.activitypub.resolve_remote_id", return_value=self.rat_user
        ):
            alias = reeltalk_import_job.is_alias(
    ...
    def test_status_already_exists(self):
        ...
        exists = reeltalk_import_job.status_already_exists(self.local_user, parsed)
        ...
        exists_two = reeltalk_import_job.status_already_exists(
    ...
    def test_create_book_from_json(self):
        ...
        book = reeltalk_import_job.create_book_from_json(book_data)
