...
from reeltalk.activitypub import Follow, parse
from reeltalk.settings import DOMAIN, NETLOC
from reeltalk.signatures import create_key_pair, make_signature, make_digest
...
class Signature(TestCase):
    ...
    def send_test_request(
        ...
        with (
            patch("reeltalk.views.inbox.activity_task.apply_async"),
    ...
    def test_correct_signature(self):
        ...
        with patch("reeltalk.models.relationship.UserFollowRequest.accept"):
    ...
    def test_remote_signer(self):
        ...
        with patch(
            "reeltalk.views.inbox.activity_task.apply_async",
            side_effect=parse(get_follow_activity(self.fake_remote, self.rat)).action(),
        ):
            with patch("reeltalk.models.user.get_remote_reviews.delay"):
                with patch(
                    "reeltalk.models.relationship.UserFollowRequest.accept"
                ) as accept_mock:
    ...
    def test_key_needs_refresh(self):
        ...
        with (
            patch("reeltalk.models.user.get_remote_reviews.delay"),
            patch(
                "reeltalk.views.inbox.activity_task.apply_async",
                side_effect=parse(
    ...
    def test_changed_data(self):
        ...
        with patch("reeltalk.activitypub.resolve_remote_id"):
