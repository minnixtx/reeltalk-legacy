...
class ActivitystreamsSignals(TestCase):
    ...
    def test_add_status_on_create_deleted(self, *_):
        """a new statuses has entered"""
        with patch("reeltalk.activitystreams.remove_status_task.delay"):
            status = models.Status.objects.create(
                user=self.remote_user, content="hi", privacy="public", deleted=True
            )
        with patch("reeltalk.activitystreams.remove_status_task.delay") as mock:
    ...
    def test_populate_streams_on_account_create_command(self, *_):
        ...
        with patch("reeltalk.activitystreams.populate_stream_task.delay") as mock:
    ...
    def test_remove_statuses_on_block(self, *_):
        ...
        with patch("reeltalk.activitystreams.remove_user_statuses_task.delay") as mock:
    ...
    def test_add_statuses_on_unblock(self, *_):
        ...
        with patch("reeltalk.activitystreams.remove_user_statuses_task.delay"):
            block = models.UserBlocks.objects.create(
                user_subject=self.local_user,
                user_object=self.remote_user,
            )

        with patch("reeltalk.activitystreams.add_user_statuses_task.delay") as mock:
            block.delete()
