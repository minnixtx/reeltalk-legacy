...
from reeltalk.views.status import find_mentions, find_or_create_hashtags
...
@patch("reeltalk.models.activitypub_mixin.ActivitypubMixin.broadcast")
class StatusViews(TestCase):
    ...
    def test_create_status_uploads(self, *_):
        """status with an uploaded image"""
        view = views.CreateStatus.as_view()
        user = self.local_user
        fake_file = SimpleUploadedFile("foo.jpg", b"a")
        upload = models.UserUpload.objects.create(
            original_name="foo.jpg", original_file=fake_file, user=user
        )
    ...
    def test_find_mentions_unknown_remote(self, *_):
        ...
        with patch("reeltalk.views.status.handle_remote_webfinger") as rwf:
...
http://www.fish.com/"""
    ...
    def test_format_links_special_chars(self, *_):
        ...
        url = "https://tech.lgbt/@reeltalk"
        self.assertEqual(
            views.status.format_links(url), f'<a href="{url}">tech.lgbt/@reeltalk</a>'
        )
    ...
    def test_format_links_ignore_non_urls(self, *_):
        ...
        text_elision = "> “The distinction is significant.” [...]"  # reeltalk#2993
        text_quoteparens = "some kind of gene-editing technology (?)"  # reeltalk#3049
    ...
    def test_delete_status(self, mock, *_):
        ...
        with patch("reeltalk.activitystreams.remove_status_task.delay") as redis_mock:
