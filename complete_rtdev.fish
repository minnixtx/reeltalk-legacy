# rt-dev auto-completions for fish-shell.
# copy this to ~/.config/fish/completions/ with the name `rt-dev.fish`
# this will only work if renamed to `rt-dev.fish`.
...
function __bw_complete -a cmds cmd desc
    complete -f -c rt-dev -n "not __fish_seen_subcommand_from $cmds" -a $cmd -d $desc
end
...
function __bw_complete_subcommand -a cmd
	complete -f -c rt-dev -n "__fish_seen_subcommand_from $cmd" $argv[2..-1]
end

__bw_complete_subcommand "up" -a "(docker-compose config --service)"
__bw_complete_subcommand "pytest" -a "reeltalk/tests/**.py"
