...
| CI checks green (ruff, mypy, ESLint, stylelint, CodeQL) | ✅ **PUSHED 2026-09-04** — all five previously-failing workflows fixed (pre-existing debt from earlier milestones, not the Crowdin work). ruff: 31 files formatted + 15 dead-code findings dropped; mypy: strict scope in reeltalk.utils (import-cycle break + ContentFile annotation); ESLint/stylelint: search suggest assets (+ one libsass-incompatible rule disabled); CodeQL: security-events permission added. Suite green: **1051 passed / 1 skipped / 1 xfailed**. Fork main = `a97fd66f0`; all six GitHub workflows green on that SHA (incl. CodeQL SARIF upload). |
...
**Pushed to fork main on 2026-09-04** after owner approval (fast-forward, no force); fork main = `a97fd66f0`. All six GitHub workflows green on that SHA: Python (pytest+ruff+mypy), Lint JavaScript, Prettier, stylelint, CodeQL (SARIF upload now succeeds — the permissions fix confirmed in CI), and Container building for release. The fork's checks are fully green again.
